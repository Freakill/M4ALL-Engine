...
bool Shader2DClass::setup(ID3D11Device* device, HWND hwnd)
{
	bool result;


	// setup the vertex and pixel shaders.
	result = setupShader(device, hwnd, L"../M4ALL-Engine/vertex_shader_2d.vs", L"../M4ALL-Engine/pixel_shader_2d.ps");
	if(!result)
	{
		return false;
	}
...
bool Shader2DClass::draw(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, 
...
bool Shader2DClass::setupShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
...
void Shader2DClass::outputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
...
bool Shader2DClass::setShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, 
											 D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, D3DXVECTOR4 pixelColor)
{
	HRESULT result;
