...
bool Shader3DClass::setup(ID3D11Device* device, HWND windowHandler)
{
	...
	result = setupShader(device, windowHandler, L"../M4ALL-Engine/vertex_shader_3d.vs", L"../M4ALL-Engine/pixel_shader_3d.ps");
	if(!result)
	{
		return false;
	}
...
bool Shader3DClass::draw(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, 
...
bool Shader3DClass::setShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, 
					   D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture,
					   D3DXVECTOR3 lightDirection, D3DXVECTOR4 diffuseColor)
{
	HRESULT result;
